...
class Graph:
    ...
    def removeNode(self,node):
        for col in range(self.nodes):
            self.adjArr[node][col] = 0
            self.adjArr[col][node] = 0
            if self.arches.__contains__((node,col)):
                self.arches.remove((node,col))
            if self.arches.__contains__((col,node)):
                self.arches.remove((col,node))
        #print(self.nodes)
        #self.nodes -= 1

    def resilienceCalculator(self, seed):
        random.seed(seed)
        deleteOrder=random.sample(range(self.nodes), self.nodes)
        resilience=[]
    ...
    def DFS_Visited(self, u, visited,idToColor):
        ...
        for i, v in enumerate(self.adjArr[u]):  #ogni v contiene gli ID dei vertici che hanno un arco con u
            #NB È QUI IL PROBLEMA, OCCORRE RIDIMENSIONARE LA MATRICE O NON CONSIDERARE SELF.NODES -= 1
            if v == 1:
